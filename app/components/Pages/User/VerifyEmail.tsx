...
const BetaAccessCode = (props) => {
  ...
  const onSubmit = async (data) => {
    axios_instance
      .post("/user/verify-beta-access/", { ...data, ...current_user })
      .then((res) => {
        console.log("res", res);
        reset({});
    ...
    <div className="flex flex-col items-center justify-center">
      ...
      <div className="pt-4 flex items-center space-x-3 justify-center w-full ">
        ...
        <Link
          ...
          href="mailto:founders@leaptable.us?subject=Leaptable » Support request"
        >
          Contact Us{" "}
        </Link>
      </div>
    ...
    <div>
      {
        <div className="flex flex-col items-center align-middle">
          ...
          <p className={"text-xl leading-10 py-3"}>
            Leaptable is still in private beta.
          </p>
...
export default ({ user }) => {
  ...
  const has_beta_access = _get(system_user, "data.user[0].beta_access", false);
  const has_email_verified = _get(
    system_user,
    "data.user[0].email_verified",
    false
  );
  if (has_beta_access && has_email_verified) {
    router.push("/data/frame/latest");
  }
