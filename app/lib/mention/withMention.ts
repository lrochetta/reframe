...
>(
  ...
  editor.insertText = (text) => {
    ...
    if (matchesPreviousCharPattern && ["@", "/"].includes(text)) {
      let _trigger = ["@", "/"].find((t) => t === text);
      console.log(_trigger, text);
  ...
  editor.apply = (operation) => {
    ...
    if (operation.type === "insert_text" || operation.type === "remove_text") {
      ...
      if (
        !["@", "/"].includes((operation.node as TMentionInputElement).trigger)
      ) {
        return;
      }
      ...
      if (
        ...
        // const activeId = (operation.node as TMentionInputElement).trigger === '@' ? 'at_mention' : 'slash_mention';
        const activeId =
          (operation.node as TMentionInputElement).trigger === "@"
            ? "at_mention"
            : "slash_mention";
